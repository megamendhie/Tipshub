...
import com.sqube.tipshub.services.UserDataFetcher
import com.sqube.tipshub.fragments.HomeFragment
import com.sqube.tipshub.fragments.RecommendedFragment
import com.sqube.tipshub.fragments.BankerFragment
import com.sqube.tipshub.fragments.NotificationFragment
import android.widget.CompoundButton
import androidx.core.view.GravityCompat
import androidx.work.WorkManager
import androidx.work.PeriodicWorkRequest
import com.sqube.tipshub.services.NotificationCheckWorker
import com.sqube.tipshub.services.DailyNotificationWorker
...
class MainActivity : AppCompatActivity(), BottomNavigationView.OnNavigationItemSelectedListener,
    View.OnClickListener, NavigationView.OnNavigationItemSelectedListener, HeaderActions {
    ...
    override fun headerActionClick(pos: Int) {
        if(pos==0) binding.drawerLayout.openDrawer(GravityCompat.START)
        else if( pos==3){
            binding.drawerLayout.closeDrawer(GravityCompat.START)
            supportActionBar?.title = "Recommended"
            fragmentManager.beginTransaction().hide(fragmentActive).show(fragmentRec).commit()
            fragmentActive = fragmentRec
            val item = binding.bottomNavigation.menu.findItem(R.id.nav_recommended)
            item.isChecked = true
        }
    }
}